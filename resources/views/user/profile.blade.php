<x-layout title="Profile: {{ $user->name }}">
    ...
    <div class="p-5">
        <div class="flex justify-between items-center">
            ...
            @if(auth()->check() && $user->id === auth()->user()->id)
            <a href="/user/{{ $user->username }}/contact/create" class="text-gray-600 hover:bg-gray-500 hover:text-white rounded-md px-3 py-1 text-sm font-medium">Add Contact</a>
            @endif
        </div>
        @php
        ...
        <div class="flex justify-between items-center">
            ...
            @if(auth()->check() && $user->id === auth()->user()->id)
            <a href="/user/{{ $user->username }}/project/create" class="text-gray-600 hover:bg-gray-500 hover:text-white rounded-md px-3 py-1 text-sm font-medium">Add Project</a>
            @endif
        </div>
        @php
            $contacts = $user->relation('contact','M:M')->get();
